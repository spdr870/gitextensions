        ...
        private void BuildOrderedRowCache(IList<RevisionGraphRevision> orderedNodesCache, int currentRowIndex, int lastToCacheRowIndex)
        {
            ...
            while (nextIndex <= lastToCacheRowIndex && cacheCount > nextIndex)
            {
                ...
                if (nextIndex == 0)
                {
                    ...
                    {
                        startSegment.LaneInfo = new LaneInfo(startSegment.Child, startSegment.Child, null);
                    ...
                    foreach (var segment in previousRevisionGraphRow.Segments.Where(s => s.Parent != previousRevisionGraphRow.Revision))
                    {
                        ...
                        if (revision == segment.Parent)
                        {
                            ...
                            foreach (var startSegment in revision.StartSegments)
                            {
                                ...
                                {
                                    if (startSegment.LaneInfo == null || startSegment.LaneInfo.StartScore > segment.LaneInfo.StartScore)
                                    {
                                ...
                                else
                                {
                                    ...
                                    {
                                        startSegment.LaneInfo = new LaneInfo(startSegment.Parent, startSegment.Child, segment.LaneInfo);
                                        startSegment.LaneInfo.IsMergeLane = startSegment.Parent.Children.Count > 1 && startSegment.Child.Parents.Count > 1;
                                    }
                                }
                            }
